...
public class Program {
	...
	private void validateParam(int pname, String errFormat) {
		...
		if (GL_FALSE == errc[0]) {
			...
			throw new IllegalStateException(String.format("%s %s", errFormat, glGetProgramInfoLog(id, errc[1])));
		}
	}
	...
	/**
	 * Creates new OpenGL video buffer, i.e. allocates video memory and initialize
	 * it with the data
	 * 
	 ...
	 *                   <code>{px,py,pz}{nz,ny,nz}</code> format would be 6 since 3
	 *                   floats for position and 3 floats for normal vector
	 * @param offset     - offset from vertex attribute component begin,
	 *                   <code>{px,py,pz}{nz,ny,nz}</code> format should be 0 when
	 *                   binding position and 3 when binding normal vector
	 */
	public void passVertexAttribArray(int attrNo, VideoBuffer vbo, boolean normalized, int vertexSize, int stride,
	    int offset) {
		...
		vbo.bind();
		glVertexAttribPointer(attrNo, vertexSize, vbo.getDataType().glEnum(), normalized, byteStride, componentOffset);
		validateOpenGL();
	...
	/**
	 * Deletes program object and free all associated resources. Should be called manually to prevent video memory leaks 
	 */
	public void delete() {
