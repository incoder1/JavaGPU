  ...
  // Fragment shader resource
	...
	public static void main(String[] args) {
		...
		try (Window window = new Window(800, 600, "Cube")) {
			...
			final Program program = Program.builder().setVertex(Shader.load(VERTEX.openStream(), Shader.Type.VERTEX))
   	...
   	// material adse matrix uniform location
		...
		public Scene(final Program program) {
			...
			try (MemoryStack stack = MemoryStack.stackPush()) {
				...
				VideoBuffer vbo = program.createVideoBuffer(stack.floats(VERTEX), VideoBuffer.Type.ARRAY_BUFFER,
				    VideoBuffer.Usage.STATIC_DRAW);
				
				// Create vertex index buffer object
				VideoBuffer vio = program.createVideoBuffer(stack.shorts(INDECIES), VideoBuffer.Type.ELEMENT_ARRAY_BUFFER, VideoBuffer.Usage.STATIC_DRAW);
		...
		public void render(int width, int height) {
			...
			try (MemoryStack stack = MemoryStack.stackPush()) {
				...
				glBindVertexArray(vao);				
				nglDrawElements(GL11.GL_TRIANGLES, INDECIES.length, GLType.UNSIGNED_SHORT.glEnum() , MemoryUtil.NULL );
				glBindVertexArray(0);
